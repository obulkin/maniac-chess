...
RSpec.describe Rook, type: :model do
  describe "#valid_move?" do
    ...
    it "should properly evaluate malformed moves that are otherwise valid" do
      ...
      expect(white_rook.valid_move?(0, 2)).to eq(false)
      expect(white_rook.valid_move?(2, 0)).to eq(false)
      expect(white_rook.valid_move?(-8, -9)).to eq(false)
      expect(white_rook.valid_move?(-1, -9)).to eq(false)
      expect(white_rook.valid_move?(1, -8)).to eq(false)
      expect(white_rook.valid_move?(1, 1)).to eq(false)
      expect(black_rook.valid_move?(7, 0)).to eq(false)
      ...
      expect(black_rook.valid_move?(9, 2)).to eq(false)
      expect(black_rook.valid_move?(8, 1)).to eq(false)
      expect(black_rook.valid_move?(15, -9)).to eq(false)
      expect(black_rook.valid_move?(9, -9)).to eq(false)
      expect(black_rook.valid_move?(8, -8)).to eq(false)
    end
